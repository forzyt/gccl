...
(function($) {
  ...
  $.touchEvents = {
      start: $.support_weui.touch ? 'touchstart' : 'mousedown',
      move: $.support_weui.touch ? 'touchmove' : 'mousemove',
      end: $.support_weui.touch ? 'touchend' : 'mouseup'
  };
...
+ function($) {
  ...
  $.modal = function(params, onOpen) {
    if($(".weui_mask").length>0){$(".weui_mask").remove()};
    if($(".weui_dialog").length>0){$(".weui_dialog").remove()};
  ...
  $.closeModal = function() {
    ...
    //$(".weui_dialog_visible").removeClass("weui_dialog_visible").transitionEnd(function() {
      //$(this).remove();
    //});
    $(".weui_mask_visible").removeClass("weui_mask_visible");
    $(".weui_dialog_visible").removeClass("weui_dialog_visible");
    setTimeout(function(){if($(".weui_mask").length>0){$(".weui_mask").remove()}},200);
    setTimeout(function(){if($(".weui_dialog").length>0){$(".weui_dialog").remove()}},200);
  };
  ...
  $.prompt = function(text, title, onOK, onCancel, input,placeholder) {
    ...
    var modal = $.modal({
      text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="text" class="weui_input weui-prompt-input" id="weui-prompt-input" placeholder="' + (config.placeholder || '') + '"value="' + (config.input || '') + '" />',
      title: config.title,
      autoClose: false,
      buttons: [
      {
...
+ function($) {
  ...
  var hide = function(callback) {
    ...
    setTimeout(function(){if($(".weui_toast").length>0){$(".weui_toast").remove()}},300);
    if($(".weui_mask_transparent").length>0){$(".weui_mask_transparent").remove()};
  }

  $.toast = function(text, style, callback) {
    if($(".weui_mask_transparent").length>0){$(".weui_mask_transparent").remove()};
    if($(".weui_toast").length>0){$(".weui_toast").remove()};
